...
const writeString = (target, offset, length, value) => {
    if (value.includes("\0"))
        throw posixError("EINVAL", "Tar fields cannot contain NUL bytes.");
    const bytes = encoder.encode(value);
    if (bytes.byteLength > length)
        throw posixError("ENAMETOOLONG", "Tar header field is too long.");
    ...
    for (const entry of entries) {
        ...
        if (encoder.encode(name).byteLength > 100) {
            const split = Array.from(name.matchAll(/\//g))
                ...
                .reverse()
                .find((index) => encoder.encode(name.slice(0, index)).byteLength <= 155 &&
...
const readOctal = (source, offset, length) => {
    const digits = readString(source, offset, length).trim() || "0";
    if (!/^[0-7]+$/.test(digits))
        throw posixError("EINVAL", "Invalid octal tar field.");
    const value = Number.parseInt(digits, 8);
    if (!Number.isSafeInteger(value))
        throw posixError("EINVAL", "Tar numeric field exceeds the safe integer range.");
    return value;
};
const checksum = (header) => header.reduce((total, byte, index) => total + (index >= 148 && index < 156 ? 32 : byte), 0);
export const decodeTar = (archive) => {
    if (archive.byteLength % blockSize !== 0)
        throw posixError("EINVAL", "Tar archive is not block-aligned.");
    const entries = [];
