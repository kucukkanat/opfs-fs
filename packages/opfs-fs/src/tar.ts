...
export type TarEntry = Readonly<{ path: string; data: Uint8Array; mode: number; mtime: Date; type: "file" | "directory" | "symlink"; linkTarget?: string }>
...
export const encodeTar = (entries: readonly TarEntry[]): Uint8Array => {
  ...
  for (const entry of entries) {
    if (encoder.encode(entry.path).byteLength > 100) throw posixError("ENAMETOOLONG", `Tar path is too long: ${entry.path}`)
    ...
    if (entry.type === "symlink") writeString(header, 157, 100, entry.linkTarget ?? "")
...
const readString = (source: Uint8Array, offset: number, length: number): string => decoder.decode(source.subarray(offset, offset + length)).replace(/\0.*$/, "")
const readOctal = (source: Uint8Array, offset: number, length: number): number => Number.parseInt(readString(source, offset, length).trim() || "0", 8)
...
export const decodeTar = (archive: Uint8Array): TarEntry[] => {
  ...
  for (let offset = 0; offset + blockSize <= archive.byteLength;) {
    ...
    if (!path || (typeByte !== 0 && typeByte !== 48 && typeByte !== 50 && typeByte !== 53)) throw posixError("EINVAL", "Unsupported tar entry.")
    offset += blockSize
    const dataEnd = offset + size
    if (dataEnd > archive.byteLength) throw posixError("EINVAL", "Truncated tar archive.")
