...
export default function JustBashDemo() {
    ...
    void openWorkspace({ name: demoWorkspaceName("just-bash"), root: "/workspace" }).then(
      (opened) => {
        ...
        const shell = new Bash({
          fs: createJustBashFileSystem(opened),
          cwd: "/workspace",
      ...
      (error: unknown) => {
        if (active) setOutput(formatError(error))
      },
    )
    return () => {
      active = false
      workspaceRef.current?.close()
    }
  }, [])
  ...
  return (
    <section data-testid="just-bash-demo">
      <p>
        <button
          type="button"
          data-testid="just-bash-demo-run"
          disabled={!bash}
          onClick={() => {
            void run()
          }}
        >
          Run durable shell command
        </button>
      </p>
      <pre data-testid="just-bash-demo-output">
        <code>{output}</code>
      </pre>
    </section>
  )
